...
// -------------------------------
  ...
  async function validarValor(valor) {
  ...
  function deleteFutureRecurring(recurrenceId, fromMonthId) {
    ...
    months.filter((m) => m >= fromMonthId).forEach((m) => {
      ...
      const filtered = list.filter((t) => t.recurrenceId !== recurrenceId);
      if (filtered.length !== list.length) Core.tx.save(userId(), m, filtered);

      // limpeza do marcador "applied" (opcional)
      try {
        const k = Core.keys.recorrApplied(userId(), m);
        const applied = Core.storage.getJSON(k, []);
        const next = (applied || []).filter((rid) => rid !== recurrenceId);
        Core.storage.setJSON(k, next);
      } catch {}
    });
